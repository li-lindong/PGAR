...
from utils import AverageMeter, AccumulateMeter
...
def train_net(cfg):
    ...
    basenet_list = {'voll': Basenet_volleyball, 'cad': Basenet_collective, 'cae': Basenet_collective}
    gcnnet_list = {'voll': GCNnet_volleyball, 'cad': GCNnet_collective, 'cae': Basenet_collective}
...
def train_volleyball(data_loader, model, device, optimizer, epoch, cfg):
    ...
    actions_meter_lld = AccumulateMeter(num_class=cfg.num_actions)
    ...
    for batch_data in data_loader:
        ...
        # Predict actions
        if cfg.actions_weights is not None:
        ...
        else: actions_loss = F.cross_entropy(actions_scores, actions_in)
        ...
        actions_meter_lld.accumulate(actions_labels.cpu().numpy(), actions_in.cpu().numpy())
    ...
    train_info={
        ...
        'actions_meter': actions_meter_lld,

...
class VolleyballDataset(data.Dataset):
    ...
    """
    def __init__(self, anns, tracks, skeletons, frames, images_path, image_size, feature_size, inference_module_name, num_boxes=12, num_before=4, num_after=4, is_training=True, is_finetune=False):
        self.anns = anns
        self.tracks = tracks
    ...
    def load_samples_sequence(self, select_frames):
        ...
        for i, (sid, src_fid, fid) in enumerate(select_frames):
            ...
            for i, track in enumerate(self.tracks[(sid, src_fid)][fid]):
                ...
                try:
                    keypoint = self.skeletons[str(sid) + str(src_fid) + str(fid) + str(center)]
                except:
                    try:
                        center[1] -= 0.5
                        keypoint = self.skeletons[str(sid) + str(src_fid) + str(fid) + str(center)]
                ...
                temp_skeletons.append(keypoint)

            if len(temp_skeletons) != self.num_boxes:
                temp_skeletons = temp_skeletons + temp_skeletons[:self.num_boxes - len(temp_skeletons)]
            temp_skeletons = np.vstack(temp_skeletons)
        ...
        # 处理fformation
        top_coordinates = self.anns[sid][src_fid]['position']
        top_coordinates = np.array(top_coordinates, dtype=np.float32)
        ...
        # orientations = np.eye(8)[orientations]
        # 在voll中，人数少了就补齐
        ...
        skeletons = np.vstack(skeletons).reshape([-1, self.num_boxes, 17, 2])
        ...
        top_coordinates = torch.stack([top_coordinates] * len(select_frames), dim=0)
        orientations = torch.from_numpy(orientations).float()
        orientations = torch.stack([orientations] * len(select_frames), dim=0)

        return images, bboxes, actions, activities, orientations, top_coordinates, skeletons
